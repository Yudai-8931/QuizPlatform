...
with st.sidebar:
    st.subheader("設定")
    levels = list(LEVEL_TO_WORDS.keys())
    st.selectbox(
        "レベル",
        levels,
        index=levels.index(st.session_state.selected_level),
        key="selected_level",
        on_change=lambda: reset_quiz_for_level(st.session_state.selected_level),
    )
